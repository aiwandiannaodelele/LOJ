...
export async function POST(
  ...
  try {
    ...
    const contestProblemId = parseInt(pid);
    ...
    if (settings && settings.submitCooldown > 0) {
      const latest = await prisma.contestSubmission.findFirst({
        where: { userId },
    ...
    // Check hourly contest submission limit
    ...
    const problem = await prisma.contestProblem.findFirst({
      where: { id: contestProblemId, contestId },
    });
    if (!problem) {

...
import { Swords, ArrowLeft, Plus, Trash2, Import, ChevronDown, ChevronUp, GripVertical, Loader2, Save } from "lucide-react";
...
export default function ContestEditPage() {
  ...
  useEffect(() => {
    // Load problem bank + contest data
    Promise.all([
      fetch("/api/admin/contest?bank=1").then((r) => r.json()),
      fetch(`/api/admin/contest/${contestId}`).then((r) => r.json()),
    ]).then(([bankData, contestData]) => {
      setProblems(bankData.problems || []);
      if (contestData.id) {
        setTitle(contestData.title || "");
        setDescription(contestData.description || "");
        setType(contestData.type || "ACM");
        setIsPublic(contestData.isPublic ?? true);
        const st = new Date(contestData.startTime);
        const et = new Date(contestData.endTime);
        setStartTime(st.toISOString().slice(0, 16));
        setEndTime(et.toISOString().slice(0, 16));
        setContestProblems(
          (contestData.problems || []).map((p: any) => ({
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold tracking-tight">编辑比赛 #{contestId}</h1>
      </div>
      ...
      <div className="rounded-xl border border-border/50 bg-card p-5 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="text-sm font-medium mb-1.5 block">密码（留空公开）</label>
            <Input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="留空则不修改" />
          </div>
          <div>
            <label className="text-sm font-medium mb-1.5 block">公开可见</label>
            <Select value={isPublic ? "true" : "false"} onValueChange={(v) => setIsPublic(v === "true")}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button onClick={handleSave} disabled={saving || !title || !startTime || !endTime || contestProblems.length === 0}>

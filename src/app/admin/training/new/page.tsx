...
import { Dumbbell, ArrowLeft, Plus, Trash2, Import, ChevronDown, ChevronUp, GripVertical } from "lucide-react";
...
export default function AdminTrainingNewPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/training?bank=1")
      .then((r) => r.json())
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold tracking-tight">创建训练题单</h1>
      </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button onClick={handleCreate} disabled={creating || !title || trainingProblems.length === 0}>
          {creating ? "创建中..." : "创建题单"}
        </Button>
      </div>
    </div>

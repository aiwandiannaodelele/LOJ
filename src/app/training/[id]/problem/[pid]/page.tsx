...
function StatusMiniBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 text-[10px] font-medium px-1.5 py-0.5 rounded-full ${c.color}`}>
      <Icon className={`h-3 w-3 ${c.iconColor}`} /> {c.label}
    </span>
  );
}
...
function JudgeResultDetail({ result, action }: { result: JudgeResult; action: "run" | "submit" }) {
  ...
  const statusMessages: Record<string, string> = {
    ...
    PAC: "部分测试用例通过", WA: action === "run" ? "运行完成，输出与期望不符" : "答案错误",
    CE: "编译失败，请检查代码语法", RE: "运行时错误（段错误/异常退出）",
    TLE: "运行超时，请优化算法复杂度", MLE: "内存超限，请减少内存使用",
  };
...
function EmptyState({ icon: Icon, text }: { icon: React.ElementType; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-muted-foreground/40 gap-2">
      <Icon className="h-8 w-8" /><p className="text-xs">{text}</p>
    </div>
  );
}
...
export default function TrainingProblemPage() {
  ...
  const trainingId = rawId ? Number(rawId) : NaN;
  const trainingProblemId = rawPid ? Number(rawPid) : NaN;

  const [training, setTraining] = useState<TrainingDetail | null>(null);
  const [problem, setProblem] = useState<TrainingProblem | null>(null);
  const [error, setError] = useState<string | null>(null);

  const storageKey = useCallback((suffix: string) => `loj_training_${trainingId}_${suffix}`, [trainingId]);

  const [code, setCode] = useState(() => {
    if (typeof window === "undefined") return CPP_TEMPLATE;
    return localStorage.getItem(storageKey(`code_${trainingProblemId}`)) || CPP_TEMPLATE;
  });
  ...
  useEffect(() => { localStorage.setItem(storageKey(`code_${trainingProblemId}`), code); }, [code, trainingProblemId, storageKey]);
  ...
  useEffect(() => {
    if (isNaN(trainingId) || isNaN(trainingProblemId)) { setError("无效的训练或题目 ID"); return; }
    fetch(`/api/training/${trainingId}`)
      .then((r) => { if (!r.ok) throw new Error("加载失败"); return r.json(); })
      .then((data: TrainingDetail) => {
        setTraining(data);
        const p = data.problems.find((p) => p.id === trainingProblemId) || null;
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(storageKey(`submit_${trainingProblemId}`));
    if (saved) {
      try {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/training/${trainingId}/problem/${problem.id}/submit`, {
        method: "POST", headers: { "Content-Type": "application/json" },
      ...
      if (typeof window !== "undefined") localStorage.setItem(storageKey(`submit_${trainingProblemId}`), JSON.stringify({ ...submitResult, lastAction: "submit" }));
    } catch {
      const failResult: JudgeResult = { status: "RE", stdout: "", stderr: "提交失败，请重试", time: 0, memory: 0 };
      setResult(failResult);
      if (typeof window !== "undefined") localStorage.setItem(storageKey(`submit_${trainingProblemId}`), JSON.stringify({ ...failResult, lastAction: "submit" }));
    } finally { setSubmitting(false); }
  }, [problem, code, language, submitting, session, router, trainingId, trainingProblemId, storageKey]);
  ...
  return (
    <div className="h-[calc(100dvh-3.5rem)] flex flex-col bg-background overflow-hidden">
      <header className="h-11 shrink-0 border-b bg-background flex items-center px-4">
        <button type="button" onClick={() => router.push(`/training/${trainingId}`)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
          <ChevronLeft className="h-3.5 w-3.5" /> 返回训练
        </button>
      </header>
